window.addEventListener("load", () => {
  ...
  function initializeScene() {
    ...
    const curvature = 35;
    const segementsX = 200;
    const segementsY = 200;
    ...
    function updateTexture(offset = 0) {
      ...
      for (let i = -extraSlides; i < extraSlides + totalSlides; i++) {
        ...
        if (img) {
          ...
          if (imgAspect > rectAspect) {
            ...
            drawX = slideRect.x + (slideRect.width - drawWidth) * 0.5;
            drawY = slideRect.y;
          } else {
            drawWidth = slideRect.width;
            drawHeight = drawWidth / imgAspect;
            drawX = slideRect.x;
            drawY = slideRect.y + (slideRect.height - drawHeight) * 0.5;
          }
          ctx.save();
          ctx.beginPath();
          ctx.roundRect(
