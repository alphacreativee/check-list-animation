...
const createLights = () => {
  ...
  shadowLight = new THREE.DirectionalLight(0xffb0b0, 0.4);
  ...
  light2 = new THREE.DirectionalLight(0xfafafa, 0.25);
  light2.position.set(-600, 350, 350);

  light3 = new THREE.DirectionalLight(0xff2e2e, 0.15);
...
const updateVertices = (time) => {
  ...
  for (let i = 0; i < bubbleGeometry.vertices.length; i++) {
    ...
    // Giảm biên độ biến dạng và giữ tỷ lệ gần 1
    let ratio = perlin * 0.1 * (dist + 0.1) + 0.95; // Giảm từ 0.3 xuống 0.1, tăng từ 0.8 lên 0.95
