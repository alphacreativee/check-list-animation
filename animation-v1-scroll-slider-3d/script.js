document.addEventListener("DOMContentLoaded", () => {
  ...
  lettersRenderer.setClearColor(0x0000000, 0);
  ...
  const createTextAnimation = (yPos, amplitude) => {
    ...
    console.log(curvePoints[0] instanceof THREE.Vector3);
  ...
  const path = [
    createTextAnimation(10, 2),
    createTextAnimation(4, 1),
    createTextAnimation(-4, -1),
    createTextAnimation(-10, -2),
  ];
  ...
  const updateLetterPositions = () => {
    letterPositions.forEach((positions, element) => {
      ...
      element.style.transform = `translate(-50%,-50%) translate3d(${positions.current.x}px , ${positions.target.y}px,0px)`;
    });
  };
